...
var width = 900;
var height = 500
var map3 = d3.select(".map-3").append("svg")
.attr("width", width)
.attr("height", height)
.style('display', 'block')
...
.defer(d3.csv, "data/arrest_rate_v6.csv", function(d){
  third_period_r.set(d.id, +d.rate0912);
...
.enter().append("path")
  .attr("fill", function(d) { return color(d.rate = third_period_r.get(d.id)); })
  .attr("d", path)
