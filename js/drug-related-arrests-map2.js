...
var width = 900;
var height = 500
var map2 = d3.select(".map-2").append("svg")
.attr("width", width)
.attr("height", height)
.style('display', 'block')
...
.defer(d3.csv, "../../data/arrest_rate_v6.csv", function(d){
  second_period_r.set(d.id, +d.rate9902);
...
.enter().append("path")
  .attr("fill", function(d) { return color(d.rate = second_period_r.get(d.id)); })
  .attr("d", path)
